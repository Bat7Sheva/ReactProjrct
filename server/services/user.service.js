...
const getUser = async (id) => {
    const users = await getData();
    const _user = await users.find(u => u.id === Number(id));
    return _user;
}
...
const getUserByUsername = async (username, password) => {
    ...
    const users = await getData();
    try {
        const _user = await users.find(u => u.username === String(username) && u.password === String(password));
        return _user.status;
    }
    catch{
        return "עדין אינך מחובר 😥";
    }
}
